...
import { DashboardData, ProdutoMaisVendido } from "@/types/dashboard.types";
...
export async function carregarDashboard(
  ...
  if (periodoSelecionado === "customizado") {
    ...
    const dFim = date.to ? new Date(date.to) : new Date(date.from);
  ...
  const mapaProdutos: Record<string, ProdutoMaisVendido> = {};
  ...
  itensVenda?.forEach((item: any) => {
    ...
    const lucroItem = (precoUnitario - precoCusto) * qtd;

    faturamento += subtotal;
    lucro += lucroItem;
    totalItensVendidos += qtd;
    //vendo o dia exato que o item foi vendido
    const dataVenda = parseISO(item.vendas.criado_em);
    //verificando se esse dia bate com a data dos ultimos 7 dias
    const dia = ultimos7Dias.find((d) => isSameDay(d.dataOriginal, dataVenda));
  ...
  const dadosGrafico = ultimos7Dias.map((d) => ({
    diaSemana: d.diaSemana.charAt(0).toUpperCase() + d.diaSemana.slice(1),
    faturamento: Number(d.faturamento.toFixed(2)),
  }));

  const produtosMaisVendidos = Object.values(mapaProdutos)
